...
export default function AdminLoginPage() {
  ...
  const handlePhoneSubmit = (e: React.FormEvent) => {
    ...
    setLoading(true);
    setNotification("Sending Admin OTP...");

    // Mock API Call for Request OTP
    setTimeout(() => {
      setLoading(false);
  ...
  const handleOtpSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!otp || otp.length < 4) {
      setNotification("Please enter a valid OTP.");
      return;
    }

    setLoading(true);
    setNotification("Verifying Admin Identity...");

    // Mock API Call for Verify OTP
    setTimeout(() => {
      setLoading(false);
      
      // Save admin token distinct from normal token
      localStorage.setItem("admin_token", "mock_admin_token_" + phone);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-950 p-4">
      ...
      >
        ...
        {/* Notification */}
        {notification && (
          <div className="p-3 bg-slate-800 border border-slate-700 rounded-xl text-center text-xs text-slate-300 font-mono flex items-center justify-center gap-2">
